...
export function convertPrimitive(avroType: string): string | null {
  switch (avroType) {
    case "long":
    case "int":
    case "double":
...
export function avroToTypeScript(
  ...
  if (isEnumType(schema)) {
    ...
    output.push(`namespace ${schemaInterfaceName ?? schema.name} {`);
    convertRecord(schema, output, {
      shouldExport: true,
      nameOverwrite: "EventOpts", // because this is namespaced, we _do_ want to repeat the top-level name
    });
    output.push("}");
  } else {
...
export function convertRecord(
  ...
  fileBuffer: string[],
  {
    shouldExport = false,
    nameOverwrite,
  }: { shouldExport?: boolean; nameOverwrite?: string } = {}
): string {
  let buffer = `${shouldExport ? "export " : ""}interface ${
    nameOverwrite ?? recordType.name
  } {\n`;
...
export function convertFieldDec(field: Field, buffer: string[]): string {
  // Union Type
  return `\t${field.name}${isOptional(field.type) ? "?" : ""}: ${convertType(
    field.type,
    buffer
  )};`;
}
